...
Label(ws, text="Param/Gráfico").grid(row=0, column=6)
Label(ws, text="Sx     ").grid(row=1, column=6)
Label(ws, text="Sy     ").grid(row=2, column=6)
Label(ws, text="Sz     ").grid(row=3, column=6)
Label(ws, text="alpha1 ").grid(row=4, column=6)
Label(ws, text="alpha2 ").grid(row=5, column=6)
Label(ws, text="a      ").grid(row=6, column=6)
Label(ws, text="b      ").grid(row=7, column=6)
Label(ws, text="m      ").grid(row=8, column=6)
Label(ws, text="m2     ").grid(row=9, column=6)
Label(ws, text="n1     ").grid(row=10, column=6)
Label(ws, text="n2     ").grid(row=11, column=6)
Label(ws, text="n3     ").grid(row=12, column=6)
Label(ws, text="Alpha1 ").grid(row=13, column=6)
Label(ws, text="Alpha2 ").grid(row=14, column=6)
Label(ws, text="A      ").grid(row=15, column=6)
Label(ws, text="B      ").grid(row=16, column=6)
Label(ws, text="M      ").grid(row=17, column=6)
Label(ws, text="M2     ").grid(row=18, column=6)
Label(ws, text="N1     ").grid(row=19, column=6)
Label(ws, text="N2     ").grid(row=20, column=6)
Label(ws, text="N3     ").grid(row=21, column=6)
...
def grelha():
    ...
    for i in range(int(Ng.get())):
        Label(ws, text=str(i + 1)).grid(row=0, column=i + 7)
        ...
        talpha1[i].grid(row=4, column=i + 7)
        talpha1[i].insert(0, "0")
        talpha2[i] = Entry(ws, width=10)
        talpha2[i].grid(row=5, column=i + 7)
        talpha2[i].insert(0, "0")
        ta[i] = Entry(ws, width=10)
        ta[i].grid(row=6, column=i + 7)
        ta[i].insert(0, "1")
        tb[i] = Entry(ws, width=10)
        tb[i].grid(row=7, column=i + 7)
        tb[i].insert(0, "1")
        tm[i] = Entry(ws, width=10)
        tm[i].grid(row=8, column=i + 7)
        tm[i].insert(0, "1")
        tm2[i] = Entry(ws, width=10)
        tm2[i].grid(row=9, column=i + 7)
        tm2[i].insert(0, "1")
        tn1[i] = Entry(ws, width=10)
        tn1[i].grid(row=10, column=i + 7)
        tn1[i].insert(0, "1")
        tn2[i] = Entry(ws, width=10)
        tn2[i].grid(row=11, column=i + 7)
        tn2[i].insert(0, "1")
        tn3[i] = Entry(ws, width=10)
        tn3[i].grid(row=12, column=i + 7)
        tn3[i].insert(0, "1")
        tAlpha1[i] = Entry(ws, width=10)
        tAlpha1[i].grid(row=13, column=i + 7)
        tAlpha1[i].insert(0, "0")
        tAlpha2[i] = Entry(ws, width=10)
        tAlpha2[i].grid(row=14, column=i + 7)
        tAlpha2[i].insert(0, "0")
        tA[i] = Entry(ws, width=10)
        tA[i].grid(row=15, column=i + 7)
        tA[i].insert(0, "1")
        tB[i] = Entry(ws, width=10)
        tB[i].grid(row=16, column=i + 7)
        tB[i].insert(0, "1")
        tM[i] = Entry(ws, width=10)
        tM[i].grid(row=17, column=i + 7)
        tM[i].insert(0, "1")
        tM2[i] = Entry(ws, width=10)
        tM2[i].grid(row=18, column=i + 7)
        tM2[i].insert(0, "1")
        tN1[i] = Entry(ws, width=10)
        tN1[i].grid(row=19, column=i + 7)
        tN1[i].insert(0, "1")
        tN2[i] = Entry(ws, width=10)
        tN2[i].grid(row=20, column=i + 7)
        tN2[i].insert(0, "1")
        tN3[i] = Entry(ws, width=10)
        tN3[i].grid(row=21, column=i + 7)
        tN3[i].insert(0, "1")
    ...
    def grafico():
        for i in range(int(Ng.get())):
            ...
            alpha1[i] = (float(talpha1[i].get()))
            alpha2[i] = (float(talpha2[i].get()))
            a[i] = (float(ta[i].get()))
            b[i] = (float(tb[i].get()))
            m[i] = (float(tm[i].get()))
        ...
        if int(Tipo.get()) == 3:
            angle = np.linspace(-np.pi, np.pi, int(reso.get()))  # Max rec. 141 em cada e ser numero impar por causa da simetria
            angle2 = np.linspace(0, np.pi * 2, int(reso2.get()))
                ...
                for i1 in range(Matriz[0]):
                    for i2 in range(Matriz[1]):
                        ...
                        if i <= len(a) - 1:
                            def r1(fi):
                                r11 = 1 / (
                                        (((abs(np.cos(m[i] * (fi - alpha1[i]) / 4) / (a[i] ** (n2[i])))) ** (n2[i])) + (
                                                (abs(np.sin(m2[i] * (fi - alpha2[i]) / 4) / (b[i] ** (n3[i])))) ** (
                                            n3[i]))) ** (
                                                1 / n1[i]))
                                if int(Logaritr.get()) == 1:
                                    r11 = r11 * (((angle + 3 * np.pi) / (2 * np.pi)) ** intensr)
                                return r11
                    ...
                    for i2 in range(Matriz[1]):
                        ...
                        if i <= len(a) - 1:
                            ...
                            ax = fig.add_subplot(Matriz[0], Matriz[1], i + 1,
                                                 projection='3d')  # fig.gca(projection = '3d')
                            maxs = max(Sx[i], Sy[i], Sz[
                                i])  # max([max(X),b,max(Y),abs(min(Y)),max(Z),abs(min(Z))]) Não funciona por ser 3d
                            ax.set_xlim3d(-maxs, maxs)
                            ax.set_ylim3d(-maxs, maxs)
                            ax.set_zlim3d(-maxs, maxs)
                            ax.plot_surface(X, Y, Z, color=str(cl3.get()), rstride=1, cstride=1)
        ...
        if int(Tipo.get()) == 2:
            theta = np.linspace(0, np.pi * 2, int(reso.get()))
...
# Verificação se o numero de formas cabe na tabela
...
lab_Graf = Label(ws, text="Parametros de Forma").grid(row=6, column=1)

toroidal = IntVar()
lab_Tor = Label(ws, text="Toroidal").grid(row=7, column=0)
op_tyes = Radiobutton(ws, text="Sim", indicatoron=0, variable=toroidal, value=1).grid(row=7, column=2)
op_tno = Radiobutton(ws, text="Não", indicatoron=0, variable=toroidal, value=0).grid(row=7, column=3)

Logaritr = IntVar()
lab_Exr = Label(ws, text="Exponencial radial?").grid(row=8, column=0)
op_eryes = Radiobutton(ws, text="Sim", indicatoron=0, variable=Logaritr, value=1,
                       command=lambda: [lab_ir.grid(row=10, column=1), ir.grid(row=11, column=1)]).grid(row=8, column=2)
op_erno = Radiobutton(ws, text="Não", indicatoron=0, variable=Logaritr, value=0,
                      command=lambda: [lab_ir.grid_forget(), ir.grid_forget()]).grid(row=8, column=3)

Logaritz = IntVar()
lab_Exz = Label(ws, text="Exponencial em z?").grid(row=9, column=0)
op_ezno = Radiobutton(ws, text="Não", indicatoron=0, variable=Logaritz, value=0,
                      command=lambda: [lab_ia.grid_forget(), ia.grid_forget(),
                                       lab_id.grid_forget(), id.grid_forget()]).grid(row=9, column=2)
op_ezamp = Radiobutton(ws, text="Amplitude", indicatoron=0, variable=Logaritz, value=1,
                       command=lambda: [lab_ia.grid(row=10, column=2), ia.grid(row=11, column=2),
                                        lab_id.grid_forget(), id.grid_forget()]).grid(row=9, column=3)
op_ezdesl = Radiobutton(ws, text="Deslocamento", indicatoron=0, variable=Logaritz, value=2,
                        command=lambda: [lab_id.grid(row=10, column=3), id.grid(row=11, column=3),
                                         lab_ia.grid_forget(), ia.grid_forget()]).grid(row=9, column=4)
op_exmist = Radiobutton(ws, text="Misto", indicatoron=0, variable=Logaritz, value=3,
                        command=lambda: [lab_id.grid(row=10, column=3), id.grid(row=11, column=3),
                                         lab_ia.grid(row=10, column=2), ia.grid(row=11, column=2)]).grid(row=9,
                                                                                                         column=5)
# Intensidades

lab_ir = Label(ws, text="Intensidade Radial")
lab_ir.grid(row=10, column=1)
lab_ir.grid_forget()
ir = Entry(ws, width=5)
ir.grid(row=11, column=1)
ir.insert(0, "1")
ir.grid_forget()

lab_ia = Label(ws, text="Intensidade Amplitude")
lab_ia.grid(row=10, column=2)
lab_ia.grid_forget()
ia = Entry(ws, width=5)
ia.grid(row=11, column=2)
ia.insert(0, "1")
ia.grid_forget()

lab_id = Label(ws, text="Intensidade Deslocamento")
lab_id.grid(row=10, column=3)
lab_id.grid_forget()
id = Entry(ws, width=5)
id.grid(row=11, column=3)
id.insert(0, "1")
id.grid_forget()

lab_tscale = Label(ws, text="Escala")
lab_tscale.grid(row=12, column=0)
scale = Entry(ws, width=5)
scale.grid(row=12, column=1)
scale.insert(0, "1")

Label(ws, text="Parametros da Imagem").grid(row=13, column=1)

lab_tpatht = Label(ws, text="Path")
lab_tpatht.grid(row=14, column=0)
patht = Entry(ws)
patht.grid(row=14, columnspan=2, sticky='e')
patht.insert(0, "C:Desktop/")

Transp = IntVar()
lab_Transp = Label(ws, text="Fundo").grid(row=15, column=0)
op_tpyes = Radiobutton(ws, text="Transparente", indicatoron=0, variable=Transp, value=0)
op_tpyes.grid(row=15, column=1)
op_tpno = Radiobutton(ws, text="Branco", indicatoron=0, variable=Transp, value=1)
op_tpno.grid(row=15, column=2)

lab_tformt = Label(ws, text="Formato")
lab_tformt.grid(row=16, column=0)
formt = Entry(ws, width=5)
formt.grid(row=16, column=1)
formt.insert(0, "svg")

lab_tdpi = Label(ws, text="DPI")
lab_tdpi.grid(row=17, column=0)
dpi = Entry(ws, width=5)
dpi.grid(row=17, column=1)
dpi.insert(0, "2400")

lab_cl2 = Label(ws, text="Cor Linha 2D")
lab_cl2.grid(row=18, column=0)
cl2 = Entry(ws, width=5)
cl2.grid(row=18, column=1)
cl2.insert(0, "grey")

lab_cl3 = Label(ws, text="Cor da Sup. 3D")
lab_cl3.grid(row=19, column=0)
cl3 = Entry(ws, width=5)
cl3.grid(row=19, columnspan=3)
cl3.insert(0, "w")
